...
typedef struct Trajectory_Point {
	...
	Trajectory_Point(glm::vec3 init_pos, float a_of_parabola, float b_of_parabola) {

		pos = init_pos;
		float scalar_der = 2 * a_of_parabola * pos.x + b_of_parabola;
		tangent = glm::normalize(glm::vec3(1, scalar_der, 0));
		angle = atan(scalar_der);
	}

	Trajectory_Point(glm::vec3 new_pos, glm::vec3 new_tan, float new_angle) {
	...
	void printTrajectoryPoint() {

		printf("\nPos: %f %f %f		Tang: %f %f %f		Angle: %f in gradi\n", pos.x, pos.y, pos.z, tangent.x, tangent.y, tangent.z, glm::degrees(angle));
	}
...
typedef struct Trajectory {
	...
	Trajectory(std::vector<Trajectory_Point> init_trajectory, glm::vec3 init_proj_direction_on_xz) {
		...
		for (int i = 0; i < trajectory.size() - 1; i++) {
			...
			approx_arc_length += glm::length(trajectory[i].pos - trajectory[i + 1].pos);
...
typedef std::vector<Trajectory_Point> VecTrajectoryPoints;


Trajectory transformTrajectory(Trajectory old_trajectory, glm::mat4 mat_trans) {

	VecTrajectoryPoints newTrajectory;

	for (Trajectory_Point p : old_trajectory.trajectory) {

		glm::vec3 newPos = (mat_trans * glm::vec4(p.pos, 1)).xyz;
		glm::vec3 newTan4 = (mat_trans * glm::vec4(p.tangent, 1));
		glm::vec3 newTan = glm::normalize(glm::vec3(newTan4.x, newTan4.y, newTan4.z));
		newTrajectory.push_back(Trajectory_Point(newPos, newTan, p.angle));
...
Trajectory calculateTrajectory(glm::vec3 source_point, glm::vec3 destination_point, float a_of_parabola, int num_of_points) {
	...
	glm::mat4 trans_source_in_center = glm::translate(glm::mat4(1), glm::vec3(-source_point.x, -source_point.y, -source_point.z));

	glm::vec4 dest_trans = trans_source_in_center * glm::vec4(destination_point, 1);



	glm::vec3 proj_norm_dest_trans = glm::normalize(glm::vec3(dest_trans.x, dest_trans.y, dest_trans.z) - (dot(glm::vec3(dest_trans.x, dest_trans.y, dest_trans.z), uy) * uy));


	float angle = acos(dot(proj_norm_dest_trans, ux));


	//se la differenza delle z è negative invertire l'angolo
	if (destination_point.z - source_point.z > 0) {
		rot = glm::rotate(glm::mat4(1), angle, uy);
	}
	else {
		rot = glm::rotate(glm::mat4(1), - angle, uy);
	}
	...
	glm::mat4 trans_mat = rot * trans_source_in_center;

	glm::mat4 inverse_trans_mat = glm::inverse(trans_mat);


	glm::vec3 dest_on_xy = (trans_mat * glm::vec4(destination_point, 1)).xyz;

	glm::vec4 dest_on_xy_4 = glm::vec4(dest_on_xy, 1);

	/*printf("\nSource: %f %f %f		Destination: %f %f %f   Angle:	%f\n", source_point.x, source_point.y, source_point.z, destination_point.x, destination_point.y, destination_point.z, glm::degrees(angle));
	printf("\nS_xy:   0 0 0		D_xy:        %f %f %f\n", dest_on_xy.x, dest_on_xy.y, dest_on_xy.z);
	printf("\nDestination after inverse matrix: %f %f %f\n", (inverse_trans_mat * dest_on_xy_4).x, (inverse_trans_mat * dest_on_xy_4).y, (inverse_trans_mat * dest_on_xy_4).z);
	printf("\nLength before: %f		Length after: %f", glm::length(destination_point - source_point), glm::length(dest_on_xy));*/




	//y = ax^2 + bx
	//l'utente deve darmi la a


	float b_of_parabola = (dest_on_xy.y - a_of_parabola * pow(dest_on_xy.x, 2)) / dest_on_xy.x;


	std::vector<Trajectory_Point> trajectory_points_vec_on_xy;

	float distance_between_points = glm::length(dest_on_xy) / num_of_points;
	...
	for (int i = 0; i < num_of_points; i++) {
		...
		x = i * distance_between_points;

		y = a_of_parabola * x * x + b_of_parabola * x;

		Trajectory_Point newPoint = Trajectory_Point(glm::vec3(x, y, 0), a_of_parabola, b_of_parabola);

		trajectory_points_vec_on_xy.push_back(newPoint);
...
private:
    ...
    void createInstance() {
    	...
    	appInfo.pApplicationName = "Project CG REALE-STAGNOLI";
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	//**************************************************************************
	//SIMILAR STRUCTURE TO MYPROJECT.CPP
	...
	// Here you can set the window parameters
	...
	// Here you destroy all the unnecessary things!
    void cleanup() {
    	cleanupSwapChain();
